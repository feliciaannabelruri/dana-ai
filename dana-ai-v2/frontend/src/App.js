...
import { checkStatus, uploadKOL, uploadInsight, uploadHomelessMedia, trainModel, getRecommendations, getLocations } from './services/apiService';
...
function BudgetSlider({ budgetMin, budgetMax, onChangeMin, onChangeMax }) {
  ...
  const pctMin = (posMin / SLIDER_MAX_POS) * 100, pctMax = (posMax / SLIDER_MAX_POS) * 100;
  const [rawMin, setRawMin] = useState(''), [rawMax, setRawMax] = useState('');
  const [focusMin, setFocusMin] = useState(false), [focusMax, setFocusMax] = useState(false);
  const clampMin = (v) => Math.min(v, maxVal - 1), clampMax = (v) => Math.max(v, minVal + 1);
  const handleSliderMin = (e) => { const v = logToValue(parseInt(e.target.value)); if (v < maxVal) { onChangeMin(String(v)); setRawMin(''); } };
  const handleSliderMax = (e) => { const v = logToValue(parseInt(e.target.value)); if (v > minVal) { onChangeMax(String(v)); setRawMax(''); } };
  const handlePresetClick = (p) => { onChangeMin(String(p.min)); onChangeMax(String(p.max)); setRawMin(''); setRawMax(''); };
  const handleRawMin = (e) => { const r = e.target.value.replace(/[^0-9]/g,''); setRawMin(r); const n = parseInt(r); if (r && n > 0) onChangeMin(String(clampMin(n))); };
  const handleRawMax = (e) => { const r = e.target.value.replace(/[^0-9]/g,''); setRawMax(r); const n = parseInt(r); if (r && n > 0) onChangeMax(String(clampMax(n))); };
  const trackStyle = { background: `linear-gradient(to right, rgba(255,255,255,.06) 0%, rgba(255,255,255,.06) ${pctMin}%, ${tc} ${pctMin}%, ${tc} ${pctMax}%, rgba(255,255,255,.06) ${pctMax}%, rgba(255,255,255,.06) 100%)` };
  const thumbCss = `.bsl::-webkit-slider-thumb{-webkit-appearance:none;width:20px;height:20px;border-radius:50%;background:${tc};box-shadow:0 0 0 3px ${tc}35,0 2px 8px rgba(0,0,0,.7);cursor:pointer;transition:box-shadow .15s,background .25s}.bsl::-webkit-slider-thumb:hover{box-shadow:0 0 0 7px ${tc}28}.bsl::-moz-range-thumb{width:20px;height:20px;border-radius:50%;border:none;background:${tc};box-shadow:0 0 0 3px ${tc}35}.bsl{-webkit-appearance:none;appearance:none;width:100%;height:6px;border-radius:3px;outline:none;cursor:pointer;background:transparent;pointer-events:none}.bsl::-webkit-slider-thumb{pointer-events:all}.bsl::-moz-range-thumb{pointer-events:all}`;
  const inputStyle = (focused) => ({ background:'transparent', border:'none', borderBottom:`1.5px solid ${focused ? tc : 'rgba(255,255,255,.1)'}`, color:'#fff', fontSize:13, fontWeight:700, fontFamily:"'Syne', sans-serif", outline:'none', padding:'2px 0', width:'100%', transition:'border-color .2s' });
  return (
    <div style={{ display:'flex', flexDirection:'column', gap:10 }}>
      <div style={{ display:'flex', gap:5, flexWrap:'wrap' }}>
        {BUDGET_PRESETS.map((p) => {
          const isActive = p.min === minVal && p.max === maxVal;
          const isClose  = !isActive && p.min <= maxVal && p.max >= minVal;
          return (<button key={p.label} onClick={()=>handlePresetClick(p)} style={{ background:isActive?p.color+'20':isClose?p.color+'0a':'rgba(255,255,255,.02)', border:`1.5px solid ${isActive?p.color+'cc':isClose?p.color+'33':'rgba(255,255,255,.06)'}`, color:isActive?p.color:isClose?p.color+'aa':'#3a3a4a', borderRadius:10, padding:'5px 12px', fontSize:11, fontWeight:700, cursor:'pointer', fontFamily:'inherit', transition:'all .18s', lineHeight:1.35, display:'flex', flexDirection:'column', alignItems:'center', gap:1, transform:isActive?'scale(1.05)':'scale(1)' }}><span style={{ fontSize:11 }}>{p.label}</span><span style={{ fontSize:9, opacity:.7, fontWeight:500 }}>{p.desc}</span></button>);
        })}
      </div>
      <div style={{ position:'relative' }}>
        <style>{thumbCss}</style>
        <div style={{ position:'absolute', top:'50%', transform:'translateY(-50%)', left:0, right:0, height:6, borderRadius:3, ...trackStyle, transition:'background .2s', pointerEvents:'none' }} />
        <div style={{ position:'relative', height:24, display:'flex', alignItems:'center' }}>
          <input type="range" className="bsl" min={0} max={SLIDER_MAX_POS} value={posMin} onChange={handleSliderMin} style={{ position:'absolute', width:'100%', zIndex:posMin>SLIDER_MAX_POS*0.8?5:4 }} />
          <input type="range" className="bsl" min={0} max={SLIDER_MAX_POS} value={posMax} onChange={handleSliderMax} style={{ position:'absolute', width:'100%', zIndex:posMin>SLIDER_MAX_POS*0.8?4:5 }} />
        </div>
        <div style={{ position:'relative', height:18, marginTop:3 }}>
          {BUDGET_PRESETS.map((p) => { const mid = Math.round((p.min+p.max)/2); const lp = (valueToLog(mid)/SLIDER_MAX_POS)*100; const inRange = p.min<=maxVal&&p.max>=minVal; return (<span key={p.label} style={{ position:'absolute', left:`${lp}%`, transform:'translateX(-50%)', color:inRange?tc:'#252535', fontSize:9, fontWeight:700, whiteSpace:'nowrap', transition:'color .2s', pointerEvents:'none' }}>{p.label}</span>); })}
        </div>
      ...
      <div style={{ display:'grid', gridTemplateColumns:'1fr auto 1fr', gap:8, alignItems:'center' }}>
        <div style={{ background:focusMin?'rgba(255,255,255,.04)':'rgba(255,255,255,.02)', border:`1.5px solid ${focusMin?tc+'99':'rgba(255,255,255,.08)'}`, borderRadius:10, padding:'10px 12px', display:'flex', flexDirection:'column', gap:3, transition:'all .2s' }}>
          <span style={{ color:'#3a3a4a', fontSize:9, fontWeight:700, letterSpacing:'.7px', textTransform:'uppercase' }}>Min Budget</span>
          <span style={{ color:tc, fontFamily:"'Syne',sans-serif", fontSize:13, fontWeight:800 }}>{fmtBudget(minVal)}</span>
          <input type="text" inputMode="numeric" placeholder="contoh: 5000000" value={rawMin} onChange={handleRawMin} onFocus={()=>{setFocusMin(true);setRawMin(String(Math.round(minVal)));}} onBlur={()=>{setFocusMin(false);setRawMin('');}} style={inputStyle(focusMin)} />
        ...
        <div style={{ background:focusMax?'rgba(255,255,255,.04)':'rgba(255,255,255,.02)', border:`1.5px solid ${focusMax?tc+'99':'rgba(255,255,255,.08)'}`, borderRadius:10, padding:'10px 12px', display:'flex', flexDirection:'column', gap:3, transition:'all .2s' }}>
          <span style={{ color:'#3a3a4a', fontSize:9, fontWeight:700, letterSpacing:'.7px', textTransform:'uppercase' }}>Max Budget</span>
          <span style={{ color:tc, fontFamily:"'Syne',sans-serif", fontSize:13, fontWeight:800 }}>{fmtBudget(maxVal)}</span>
          <input type="text" inputMode="numeric" placeholder="contoh: 50000000" value={rawMax} onChange={handleRawMax} onFocus={()=>{setFocusMax(true);setRawMax(String(Math.round(maxVal)));}} onBlur={()=>{setFocusMax(false);setRawMax('');}} style={inputStyle(focusMax)} />
          <span style={{ color:'#2a2a3a', fontSize:8 }}>custom angka Rp</span>
        </div>
      </div>
      <div style={{ background:tc+'0a', border:`1px solid ${tc}22`, borderRadius:8, padding:'8px 12px', display:'flex', justifyContent:'space-between', alignItems:'center' }}>
        <span style={{ color:'#444', fontSize:10 }}>Range dipilih</span>
        <span style={{ color:'#fff', fontFamily:"'Syne',sans-serif", fontWeight:800, fontSize:13 }}>{fmtBudget(minVal)} <span style={{ color:tc }}>—</span> {fmtBudget(maxVal)}</span>
      ...
      <div style={{ display:'flex', gap:2, height:3, borderRadius:3, overflow:'hidden' }}>
        {BUDGET_PRESETS.map((p) => { const inRange = p.min<=maxVal&&p.max>=minVal; return (<div key={p.label} style={{ flex:1, borderRadius:2, transition:'background .3s', background:inRange?p.color:p.color+'15' }} />); })}
      </div>
    </div>
  );
}
...
function MAEBadge({ meta }) {
  ...
  const rf = meta.metrics.random_forest, xgb = meta.metrics.xgboost;
  const mae = rf.mae, color = mae <= 0.05 ? GREEN : mae <= 0.10 ? GOLD : RED;
  return (
    <div style={{ marginTop:8, display:'flex', gap:6, flexWrap:'wrap', alignItems:'center' }}>
      <span style={{ color:'#333', fontSize:10 }}>Model Accuracy:</span>
      <span style={{ background:color+'18', border:`1px solid ${color}44`, color, borderRadius:6, padding:'2px 8px', fontSize:11, fontWeight:800, fontFamily:"'Syne',sans-serif" }}>MAE {mae.toFixed(4)}</span>
      {xgb?.mae_train && (<span style={{ background:GOLD+'18', border:`1px solid ${GOLD}44`, color:GOLD, borderRadius:6, padding:'2px 8px', fontSize:11, fontWeight:800, fontFamily:"'Syne',sans-serif" }}>ER MAE {xgb.mae_train.toFixed(2)}%</span>)}
      <span style={{ color:'#2a2a3a', fontSize:9 }}>{mae <= 0.05 ? 'Sangat baik' : mae <= 0.10 ? 'Baik' : 'Cukup'}</span>
    </div>
  );
}

function Badge({ label, color=ACCENT, icon=null }) {
  return (<span style={{ display:'inline-flex', alignItems:'center', gap:4, background:color+'22', color, border:`1px solid ${color}44`, padding:'2px 10px', borderRadius:20, fontSize:11, fontWeight:700, whiteSpace:'nowrap' }}>{icon && icon}{label}</span>);
}
function Stat({ label, value, color=ACCENT }) {
  return (<div style={{ background:'rgba(255,255,255,.04)', border:'1px solid rgba(255,255,255,.07)', borderRadius:10, padding:'10px 12px', textAlign:'center' }}><div style={{ color:'#555', fontSize:9, letterSpacing:'1.2px', marginBottom:3, textTransform:'uppercase' }}>{label}</div><div style={{ color, fontFamily:'Syne,sans-serif', fontWeight:800, fontSize:14 }}>{value}</div></div>);
}
function Bar({ score, color }) {
  const [w, setW] = useState(0);
  useEffect(() => { const t = setTimeout(()=>setW(score),150); return ()=>clearTimeout(t); }, [score]);
  return (<div style={{ height:5, background:'#1a1a2a', borderRadius:3, overflow:'hidden', margin:'6px 0' }}><div style={{ width:`${w}%`, height:'100%', background:color, borderRadius:3, transition:'width 1.1s cubic-bezier(.4,0,.2,1)' }} /></div>);
}
...
function KOLCard({ kol, rank }) {
  ...
  const contactColors = { whatsapp:{bg:'rgba(37,211,102,.12)',border:'rgba(37,211,102,.35)',text:'#25d366'}, tiktok:{bg:'rgba(255,0,80,.10)',border:'rgba(255,0,80,.3)',text:'#ff0050'}, instagram:{bg:'rgba(193,53,132,.10)',border:'rgba(193,53,132,.3)',text:'#c13584'}, profile:{bg:'rgba(193,53,132,.10)',border:'rgba(193,53,132,.3)',text:'#c13584'} };
  const cc = kol.contact_action ? (contactColors[kol.contact_action.type] || contactColors.profile) : null;
  return (
    <div style={{ background:'linear-gradient(135deg,rgba(255,255,255,.05),rgba(255,255,255,.02))', border:`1px solid ${c}33`, borderRadius:14, padding:18, position:'relative', transition:'all .25s' }} onMouseEnter={e=>{e.currentTarget.style.borderColor=c+'88';e.currentTarget.style.transform='translateY(-2px)';}} onMouseLeave={e=>{e.currentTarget.style.borderColor=c+'33';e.currentTarget.style.transform='translateY(0)';}}>
      ...
      <div style={{ display:'flex', gap:10, alignItems:'flex-start', marginBottom:12 }}>
        <div style={{ width:40, height:40, borderRadius:10, background:c+'22', display:'flex', alignItems:'center', justifyContent:'center', flexShrink:0, color:c }}>{isTiktok ? Icon.tiktok(18) : Icon.instagram(18)}</div>
        <div><div style={{ fontWeight:700, color:'#fff', fontSize:14 }}>@{kol.username}</div><div style={{ color:'#666', fontSize:11, marginTop:2 }}>{kol.category}</div></div>
      </div>
      ...
      {kol.has_real_er && kol.avg_er_pct && (<div style={{ marginBottom:10, display:'flex', alignItems:'center', gap:6, background:GREEN+'11', border:`1px solid ${GREEN}33`, borderRadius:8, padding:'6px 10px' }}><span style={{ color:GREEN, display:'flex' }}>{Icon.chart(13)}</span><span style={{ color:GREEN, fontSize:11, fontWeight:700 }}>ER Aktual: {kol.avg_er_pct}%</span><span style={{ color:'#555', fontSize:10 }}>data nyata</span></div>)}
      ...
      {kol.score_detail && (<><button onClick={()=>setOpen(o=>!o)} style={{ marginTop:10, background:'transparent', border:'1px solid rgba(255,255,255,.08)', color:'#555', borderRadius:6, padding:'5px 10px', cursor:'pointer', fontSize:11, fontFamily:'inherit', width:'100%', display:'flex', alignItems:'center', justifyContent:'center', gap:6 }}>{open?Icon.chevronUp(12):Icon.chevronDown(12)}{open?'Sembunyikan detail':'Detail scoring'}</button>{open && (<div style={{ marginTop:10, background:'rgba(0,0,0,.3)', borderRadius:8, padding:'10px 12px' }}>{Object.entries(kol.score_detail).map(([k,v])=>(<div key={k} style={{ display:'flex', justifyContent:'space-between', alignItems:'center', marginBottom:6 }}><span style={{ color:'#666', fontSize:11 }}>{k}</span><div style={{ display:'flex', alignItems:'center', gap:8 }}><div style={{ width:60, height:4, background:'#1a1a2a', borderRadius:2, overflow:'hidden' }}><div style={{ width:`${v}%`, height:'100%', background:v>=70?GREEN:v>=40?GOLD:RED, borderRadius:2 }} /></div><span style={{ color:'#aaa', fontSize:11, minWidth:32, textAlign:'right' }}>{v}%</span></div></div>))}</div>)}</>)}
      {kol.rate_card && Object.keys(kol.rate_card).length > 0 && (<div style={{ marginTop:10, background:'rgba(0,0,0,.3)', borderRadius:8, padding:'10px 12px' }}><div style={{ color:'#444', fontSize:9, letterSpacing:'1px', marginBottom:6, fontWeight:700 }}>RATE CARD</div>{Object.entries(kol.rate_card).map(([p,r])=>(<div key={p} style={{ display:'flex', justifyContent:'space-between', marginBottom:3 }}><span style={{ color:'#777', fontSize:11 }}>{p}</span><span style={{ color:GOLD, fontWeight:700, fontSize:11 }}>{fmt(r)}</span></div>))}</div>)}
      {kol.reasoning && (<div style={{ marginTop:10, background:c+'0e', border:`1px solid ${c}22`, borderRadius:8, padding:'8px 10px', display:'flex', gap:6, alignItems:'flex-start' }}><span style={{ color:c, flexShrink:0, marginTop:1 }}>{Icon.lightbulb(12)}</span><span style={{ color:'#bbb', fontSize:11, lineHeight:1.55 }}>{kol.reasoning}</span></div>)}
      <div style={{ marginTop:12, paddingTop:10, borderTop:'1px solid rgba(255,255,255,.05)' }}>
        {kol.pic_name && (<div style={{ display:'flex', alignItems:'center', gap:5, color:'#555', fontSize:10, marginBottom:8 }}>{Icon.user(11)}<span>PIC:</span><span style={{ color:'#777' }}>{kol.pic_name}</span></div>)}
        {kol.contact_action && cc ? (<a href={kol.contact_action.url} target="_blank" rel="noopener noreferrer" style={{ display:'flex', alignItems:'center', justifyContent:'center', gap:8, background:cc.bg, border:`1px solid ${cc.border}`, color:cc.text, borderRadius:8, padding:'9px 14px', fontSize:12, fontWeight:700, textDecoration:'none', fontFamily:'inherit', transition:'opacity .2s' }} onMouseEnter={e=>e.currentTarget.style.opacity='.75'} onMouseLeave={e=>e.currentTarget.style.opacity='1'}>{kol.contact_action.type==='whatsapp'&&Icon.whatsapp(14)}{kol.contact_action.type==='tiktok'&&Icon.tiktok(14)}{(kol.contact_action.type==='instagram'||kol.contact_action.type==='profile')&&Icon.instagram(14)}<span>{kol.contact_action.label}</span>{Icon.externalLink(10)}</a>) : (<div style={{ color:'#2a2a3a', fontSize:11, textAlign:'center', padding:'4px 0' }}>Tidak ada kontak</div>)}
      </div>
    </div>
  );
}
...
function HomelessMediaCard({ media, rank }) {
  ...
  const [open, setOpen] = useState(false);
  const contactColors = { whatsapp:{bg:'rgba(37,211,102,.12)',border:'rgba(37,211,102,.35)',text:'#25d366'}, instagram:{bg:'rgba(193,53,132,.10)',border:'rgba(193,53,132,.3)',text:'#c13584'}, profile:{bg:'rgba(193,53,132,.10)',border:'rgba(193,53,132,.3)',text:'#c13584'} };
  const cc = media.contact_action ? (contactColors[media.contact_action.type] || contactColors.instagram) : null;
  return (
    <div style={{ background:`linear-gradient(135deg,rgba(45,212,191,.05),rgba(45,212,191,.01))`, border:`1px solid ${c}33`, borderRadius:14, padding:18, position:'relative', transition:'all .25s' }} onMouseEnter={e=>{e.currentTarget.style.borderColor=c+'88';e.currentTarget.style.transform='translateY(-2px)';}} onMouseLeave={e=>{e.currentTarget.style.borderColor=c+'33';e.currentTarget.style.transform='translateY(0)';}}>
      <div style={{ position:'absolute', top:14, right:14, background:c, color:'#000', width:24, height:24, borderRadius:'50%', display:'flex', alignItems:'center', justifyContent:'center', fontSize:11, fontWeight:900 }}>#{rank}</div>
      ...
      <div style={{ display:'flex', gap:10, alignItems:'flex-start', marginBottom:12, marginTop:28 }}>
        <div style={{ width:40, height:40, borderRadius:10, background:c+'22', display:'flex', alignItems:'center', justifyContent:'center', flexShrink:0, color:c }}>{Icon.newspaper(18)}</div>
        <div><div style={{ fontWeight:700, color:'#fff', fontSize:14 }}>@{media.username}</div><div style={{ color:'#666', fontSize:11, marginTop:2 }}>{media.category}</div></div>
      </div>
      <div style={{ display:'flex', justifyContent:'space-between', alignItems:'center' }}>
        <span style={{ color:'#555', fontSize:10, letterSpacing:'.5px' }}>MATCH SCORE</span>
      ...
      {media.score_detail && (<><button onClick={()=>setOpen(o=>!o)} style={{ marginTop:10, background:'transparent', border:'1px solid rgba(255,255,255,.08)', color:'#555', borderRadius:6, padding:'5px 10px', cursor:'pointer', fontSize:11, fontFamily:'inherit', width:'100%', display:'flex', alignItems:'center', justifyContent:'center', gap:6 }}>{open?Icon.chevronUp(12):Icon.chevronDown(12)}{open?'Sembunyikan detail':'Detail scoring'}</button>{open && (<div style={{ marginTop:10, background:'rgba(0,0,0,.3)', borderRadius:8, padding:'10px 12px' }}>{Object.entries(media.score_detail).map(([k,v])=>(<div key={k} style={{ display:'flex', justifyContent:'space-between', alignItems:'center', marginBottom:6 }}><span style={{ color:'#666', fontSize:11 }}>{k}</span><div style={{ display:'flex', alignItems:'center', gap:8 }}><div style={{ width:60, height:4, background:'#1a1a2a', borderRadius:2, overflow:'hidden' }}><div style={{ width:`${v}%`, height:'100%', background:v>=70?GREEN:v>=40?GOLD:RED, borderRadius:2 }} /></div><span style={{ color:'#aaa', fontSize:11, minWidth:32, textAlign:'right' }}>{v}%</span></div></div>))}</div>)}</>)}
      {media.rate_card && Object.keys(media.rate_card).length > 0 && (<div style={{ marginTop:10, background:'rgba(0,0,0,.3)', borderRadius:8, padding:'10px 12px' }}><div style={{ color:'#444', fontSize:9, letterSpacing:'1px', marginBottom:6, fontWeight:700 }}>RATE CARD</div>{Object.entries(media.rate_card).map(([p,r])=>(<div key={p} style={{ display:'flex', justifyContent:'space-between', marginBottom:3 }}><span style={{ color:'#777', fontSize:11 }}>{p}</span><span style={{ color:GOLD, fontWeight:700, fontSize:11 }}>{fmt(r)}</span></div>))}</div>)}
      {media.reasoning && (<div style={{ marginTop:10, background:TEAL+'0e', border:`1px solid ${TEAL}22`, borderRadius:8, padding:'8px 10px', display:'flex', gap:6, alignItems:'flex-start' }}><span style={{ color:TEAL, flexShrink:0, marginTop:1 }}>{Icon.lightbulb(12)}</span><span style={{ color:'#bbb', fontSize:11, lineHeight:1.55 }}>{media.reasoning}</span></div>)}
      <div style={{ marginTop:12, paddingTop:10, borderTop:'1px solid rgba(255,255,255,.05)' }}>
        {media.pic_name && (<div style={{ display:'flex', alignItems:'center', gap:5, color:'#555', fontSize:10, marginBottom:8 }}>{Icon.user(11)}<span>PIC:</span><span style={{ color:'#777' }}>{media.pic_name}</span></div>)}
        {media.contact_action && cc ? (<a href={media.contact_action.url} target="_blank" rel="noopener noreferrer" style={{ display:'flex', alignItems:'center', justifyContent:'center', gap:8, background:cc.bg, border:`1px solid ${cc.border}`, color:cc.text, borderRadius:8, padding:'9px 14px', fontSize:12, fontWeight:700, textDecoration:'none', fontFamily:'inherit', transition:'opacity .2s' }} onMouseEnter={e=>e.currentTarget.style.opacity='.75'} onMouseLeave={e=>e.currentTarget.style.opacity='1'}>{media.contact_action.type==='whatsapp'&&Icon.whatsapp(14)}{Icon.instagram(14)}<span>{media.contact_action.label}</span>{Icon.externalLink(10)}</a>) : (<div style={{ color:'#2a2a3a', fontSize:11, textAlign:'center', padding:'4px 0' }}>Tidak ada kontak</div>)}
      </div>
    </div>
  );
}
...
function Spinner({ msg }) {
  return (<div style={{ minHeight:'100vh', background:BG, display:'flex', alignItems:'center', justifyContent:'center', flexDirection:'column', gap:20, fontFamily:"'Plus Jakarta Sans',sans-serif" }}><style>{`@keyframes spin{to{transform:rotate(360deg)}}@keyframes pulse{0%,100%{opacity:.6}50%{opacity:1}}`}</style><div style={{ position:'relative', width:70, height:70 }}>{[[70,ACCENT,'1s','normal'],[50,GOLD,'1.4s','reverse'],[30,PURPLE,'1.8s','normal']].map(([sz,c,dur,dir],i)=>(<div key={i} style={{ position:'absolute', top:(70-sz)/2, left:(70-sz)/2, width:sz, height:sz, borderRadius:'50%', border:'2.5px solid transparent', borderTopColor:c, animation:`spin ${dur} linear ${dir} infinite` }} />))}</div><div style={{ fontFamily:'Syne,sans-serif', color:ACCENT, fontSize:18, fontWeight:800 }}>DANA AI</div><div style={{ color:'#777', fontSize:13, animation:'pulse 2s ease infinite', textAlign:'center', maxWidth:300 }}>{msg}</div></div>);
}

const MSGS = ['Memproses kebutuhan campaign...','HuggingFace encoding query...','Semantic matching KOL vs campaign...','Mencari Homeless Media yang relevan...','Menyusun rekomendasi final...'];
...
export default function App() {
  ...
  // Load lokasi dari backend setelah status berhasil
  useEffect(() => {
  ...
  const loadLocations = async () => {
    ...
    try {
      ...
      // Fallback ke lokasi hardcoded jika endpoint belum ada (backward compat)
      setLocations([
        ...
        { value:'Yogyakarta', label:'Yogyakarta', group:'Yogyakarta' },
        { value:'Semarang', label:'Semarang', group:'Semarang' },
        { value:'Solo', label:'Solo', group:'Solo / Surakarta' },
        { value:'Malang', label:'Malang', group:'Malang' },
        { value:'Bali', label:'Bali', group:'Bali' },
        { value:'Medan', label:'Medan', group:'Sumatra' },
  ...
  // Reload lokasi setelah upload + train
  const reloadLocations = () => {
    if (status && !status.error) loadLocations();
  };
  ...
  const handleHomelessUpload = async (e) => {
    ...
    try {
      ...
      if (r.parsed) {
        ...
        reloadLocations(); // refresh lokasi setelah upload
      } else {
  ...
  const handleTrain = async () => {
    ...
    try {
      ...
      reloadLocations(); // refresh lokasi setelah train
    } catch(err) { setKolMsg('Error: ' + err.message); }
    setTraining(false);
  };
  ...
  const inp = { background:'rgba(255,255,255,.04)', border:'1px solid rgba(255,255,255,.09)', borderRadius:10, color:'#fff', padding:'11px 14px', fontSize:14, width:'100%', outline:'none', fontFamily:'inherit', boxSizing:'border-box' };
  const lbl = { color:'#666', fontSize:11, fontWeight:700, letterSpacing:'.7px', marginBottom:5, display:'block', textTransform:'uppercase' };
  ...
  if (page==='result' && result) {
    ...
    return (
      <div style={{ minHeight:'100vh', background:BG, fontFamily:"'Plus Jakarta Sans',sans-serif", color:'#fff' }}>
        <style>{`::-webkit-scrollbar{width:5px}::-webkit-scrollbar-thumb{background:#2a2a3a;border-radius:3px}@keyframes fu{from{opacity:0;transform:translateY(12px)}to{opacity:1;transform:none}}.fu{animation:fu .5s ease forwards}@keyframes spin{to{transform:rotate(360deg)}}`}</style>
        <div style={{ background:'rgba(7,7,15,.92)', backdropFilter:'blur(20px)', borderBottom:'1px solid rgba(255,255,255,.06)', padding:'14px 24px', display:'flex', alignItems:'center', justifyContent:'space-between', position:'sticky', top:0, zIndex:100 }}>
          <div style={{ display:'flex', alignItems:'center', gap:10 }}>
            <div style={{ width:28, height:28, borderRadius:7, background:`linear-gradient(135deg,${ACCENT},${PURPLE})`, display:'flex', alignItems:'center', justifyContent:'center', color:'#fff' }}>{Icon.bolt(14)}</div>
        ...
        <div style={{ maxWidth:1100, margin:'0 auto', padding:'24px 20px' }}>
          <div className="fu" style={{ marginBottom:20 }}>
            <div style={{ background:ACCENT+'12', border:`1px solid ${ACCENT}30`, borderRadius:16, padding:'20px 22px' }}>
              <div style={{ display:'flex', flexWrap:'wrap', gap:20, justifyContent:'space-between', alignItems:'flex-start' }}>
                <div>
                  <div style={{ color:ACCENT, fontSize:10, letterSpacing:'1.5px', fontWeight:700, marginBottom:6 }}>HASIL ANALISIS</div>
                ...
                <div style={{ display:'grid', gridTemplateColumns:'1fr 1fr', gap:10, minWidth:220 }}>
                  ...
                  {hasHomeless ? <Stat label="Media Est. Min" value={fmt(result.homeless_media?.estimated_cost_media_min)} color={TEAL} /> : <Stat label="Sisa Budget" value={fmt(result.budget_remaining)} color={GREEN} />}
                </div>
              </div>
            </div>
          </div>
          <div className="fu" style={{ animationDelay:'.1s', marginBottom:32 }}>
            <div style={{ display:'flex', alignItems:'center', gap:10, marginBottom:14 }}>
              ...
              <span style={{ fontFamily:'Syne,sans-serif', fontWeight:800, fontSize:17, display:'flex', alignItems:'center', gap:8 }}><span style={{ color:GOLD }}>{Icon.star(16)}</span>Rekomendasi KOL</span>
          ...
          {hasHomeless && (
            <div className="fu" style={{ animationDelay:'.2s' }}>
              <div style={{ display:'flex', alignItems:'center', gap:12, marginBottom:16 }}>
                ...
                <div style={{ display:'flex', alignItems:'center', gap:8, color:TEAL }}>{Icon.newspaper(16)}<span style={{ fontFamily:'Syne,sans-serif', fontWeight:800, fontSize:13, letterSpacing:'1px' }}>HOMELESS MEDIA</span></div>
  ...
  const statusIcon = backendErr
    ...
    : status===null ? <span style={{ color:'#555' }}>{Icon.loader(18)}</span>
    : modelReady ? <span style={{ color:GREEN }}>{Icon.checkCircle(18)}</span>
      ...
      <div style={{ maxWidth:720, margin:'0 auto', padding:'36px 20px', position:'relative', zIndex:1 }}>
        ...
        <div className="fu" style={{ textAlign:'center', marginBottom:32 }}>
          ...
          <h1 style={{ fontFamily:'Syne,sans-serif', fontSize:'clamp(22px,5vw,38px)', fontWeight:800, margin:'0 0 10px', lineHeight:1.15 }}>
            ...
            <span style={{ background:`linear-gradient(90deg,${ACCENT},${PURPLE})`, WebkitBackgroundClip:'text', WebkitTextFillColor:'transparent' }}>Semantic Matching</span>
          </h1>
        </div>
          ...
          <div style={{ background:backendErr?RED+'0a':modelReady?GREEN+'08':'rgba(255,255,255,.02)', border:`1px solid ${backendErr?RED+'30':modelReady?GREEN+'22':'rgba(255,255,255,.07)'}`, borderRadius:12, padding:'16px 18px' }}>
            <div style={{ display:'flex', alignItems:'center', gap:10, marginBottom:14 }}>
              ...
              <span style={{ fontWeight:700, fontSize:13, color:backendErr?RED:modelReady?GREEN:'#ccc' }}>
                {backendErr?'Backend tidak bisa dihubungi':status===null?'Menghubungi backend...':modelReady?`Model siap — ${meta.total_kol||0} KOL | ${meta.kol_with_er||0} ER nyata`:'Model belum dilatih'}
              </span>
            ...
            <div style={{ display:'grid', gridTemplateColumns:'1fr 1fr 1fr', gap:10, marginBottom:12 }}>
              <div style={{ background:'rgba(0,0,0,.2)', borderRadius:8, padding:'12px' }}>
                ...
                <button onClick={()=>kolRef.current.click()} disabled={!!backendErr} style={{ background:ACCENT+'20', border:`1px solid ${ACCENT}40`, color:ACCENT, borderRadius:8, padding:'7px 10px', cursor:'pointer', fontSize:11, fontWeight:700, fontFamily:'inherit', width:'100%', marginBottom:6, display:'flex', alignItems:'center', justifyContent:'center', gap:5, opacity:backendErr?.4:1 }}>{Icon.upload(12)} KOL.xlsx</button>
              ...
              <div style={{ background:'rgba(0,0,0,.2)', borderRadius:8, padding:'12px' }}>
                ...
                <button onClick={()=>insightRef.current.click()} disabled={!!backendErr} style={{ background:GREEN+'20', border:`1px solid ${GREEN}40`, color:GREEN, borderRadius:8, padding:'7px 10px', cursor:'pointer', fontSize:11, fontWeight:700, fontFamily:'inherit', width:'100%', marginBottom:6, display:'flex', alignItems:'center', justifyContent:'center', gap:5, opacity:backendErr?.4:1 }}>{Icon.chart(12)} insight.xlsx</button>
              ...
              <div style={{ background:'rgba(45,212,191,.04)', borderRadius:8, padding:'12px', border:`1px solid ${TEAL}22` }}>
                <div style={{ color:TEAL, fontSize:10, fontWeight:700, letterSpacing:'1px', marginBottom:8, display:'flex', alignItems:'center', gap:4 }}>{Icon.newspaper(10)} HOMELESS MEDIA</div>
                <input ref={homelessRef} type="file" accept=".xlsx" style={{ display:'none' }} onChange={handleHomelessUpload} />
                <button onClick={()=>homelessRef.current.click()} disabled={!!backendErr} style={{ background:TEAL+'20', border:`1px solid ${TEAL}40`, color:TEAL, borderRadius:8, padding:'7px 10px', cursor:'pointer', fontSize:11, fontWeight:700, fontFamily:'inherit', width:'100%', marginBottom:6, display:'flex', alignItems:'center', justifyContent:'center', gap:5, opacity:backendErr?.4:1 }}>{Icon.upload(12)} HomelessMedia.xlsx</button>
          ...
          <div style={{ background:'rgba(255,255,255,.02)', border:'1px solid rgba(255,255,255,.07)', borderRadius:16, padding:24 }}>
            <div style={{ display:'grid', gap:15 }}>
              ...
              <div>
                ...
                <textarea style={{...inp,minHeight:68,resize:'vertical'}} placeholder="Tujuan dan pesan utama campaign..." value={form.campaign_description} onChange={e=>setForm(f=>({...f,campaign_description:e.target.value}))} />
              </div>
              ...
              <div style={{ display:'grid', gridTemplateColumns:'1fr 1fr', gap:14 }}>
                ...
                <div>
                  <label style={{ ...lbl, display:'flex', alignItems:'center', gap:5 }}>
                    {Icon.pin(10)} Lokasi Target
                    {locations.length > 0 && (
                      <span style={{ color:'#2a2a4a', fontSize:9, fontWeight:500, textTransform:'none', letterSpacing:0, marginLeft:'auto' }}>
              ...
              {/* Budget Slider */}
              <div>
                <label style={{ ...lbl, display:'flex', alignItems:'center', gap:6 }}>{Icon.wallet(11)} Budget Range *</label>
                <div style={{ background:'rgba(0,0,0,.2)', border:'1px solid rgba(255,255,255,.06)', borderRadius:12, padding:'16px' }}>
                  <BudgetSlider budgetMin={form.budget_min} budgetMax={form.budget_max} onChangeMin={(val)=>setForm(f=>({...f,budget_min:val}))} onChangeMax={(val)=>setForm(f=>({...f,budget_max:val}))} />
                </div>
              </div>
              ...
              <div>
                <label style={{ ...lbl, display:'flex', alignItems:'center', justifyContent:'space-between' }}>
                  <span style={{ display:'flex', alignItems:'center', gap:6 }}>{Icon.chart(11)} Alokasi Budget</span>
                  <span style={{ color:'#444', fontSize:10, fontWeight:500, textTransform:'none', letterSpacing:0 }}>KOL {form.budget_kol_pct}% · Media {100-form.budget_kol_pct}%</span>
                </label>
                <div style={{ background:'rgba(0,0,0,.2)', border:'1px solid rgba(255,255,255,.06)', borderRadius:12, padding:'14px 16px' }}>
                  ...
                  <style>{`.split-slider{-webkit-appearance:none;appearance:none;width:100%;height:5px;border-radius:3px;background:transparent;outline:none;cursor:pointer}.split-slider::-webkit-slider-thumb{-webkit-appearance:none;width:18px;height:18px;border-radius:50%;background:#fff;box-shadow:0 1px 6px rgba(0,0,0,.5);cursor:pointer}.split-slider::-moz-range-thumb{width:18px;height:18px;border-radius:50%;border:none;background:#fff;cursor:pointer}`}</style>
                  <input type="range" className="split-slider" min={10} max={90} step={5} value={form.budget_kol_pct} onChange={e=>setForm(f=>({...f,budget_kol_pct:parseInt(e.target.value)}))} />
                  <div style={{ display:'grid', gridTemplateColumns:'1fr 1fr', gap:8, marginTop:10 }}>
                    <div style={{ background:GOLD+'12', border:`1px solid ${GOLD}33`, borderRadius:8, padding:'8px 12px' }}>
                      ...
                      <div style={{ color:'#fff', fontFamily:"'Syne',sans-serif", fontWeight:800, fontSize:14 }}>{fmtBudget(Math.round((parseFloat(form.budget_min)||0)*form.budget_kol_pct/100))}</div>
                      <div style={{ color:'#555', fontSize:9, marginTop:1 }}>– {fmtBudget(Math.round((parseFloat(form.budget_max)||0)*form.budget_kol_pct/100))}</div>
                    </div>
                    <div style={{ background:TEAL+'12', border:`1px solid ${TEAL}33`, borderRadius:8, padding:'8px 12px' }}>
                      ...
                      <div style={{ color:'#fff', fontFamily:"'Syne',sans-serif", fontWeight:800, fontSize:14 }}>{fmtBudget(Math.round((parseFloat(form.budget_min)||0)*(100-form.budget_kol_pct)/100))}</div>
                      <div style={{ color:'#555', fontSize:9, marginTop:1 }}>– {fmtBudget(Math.round((parseFloat(form.budget_max)||0)*(100-form.budget_kol_pct)/100))}</div>
                    </div>
                  </div>
                </div>
              </div>
              ...
              <div style={{ display:'grid', gridTemplateColumns:'1fr 1fr', gap:14 }}>
                ...
                <div>
                  <label style={{ color:TEAL, fontSize:11, fontWeight:700, letterSpacing:'.7px', marginBottom:5, textTransform:'uppercase', display:'flex', alignItems:'center', gap:5 }}>{Icon.newspaper(10)} Jumlah Homeless Media</label>
                  <div style={{ display:'flex', alignItems:'center', gap:12 }}>
              ...
              {!modelReady && (<p style={{ textAlign:'center', color:'#333', fontSize:11, margin:0, display:'flex', alignItems:'center', justifyContent:'center', gap:5 }}><span style={{ color:'#444' }}>{Icon.alertCircle(12)}</span>Latih model terlebih dahulu</p>)}
            </div>
          </div>
        </div>

        <div style={{ marginTop:16, textAlign:'center', color:'#2a2a3a', fontSize:11 }}>
          DANA AI · HuggingFace Multilingual · KOL + Homeless Media · Location-Aware
        </div>
      </div>
    </div>
  );
